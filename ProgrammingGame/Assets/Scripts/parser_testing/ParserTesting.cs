...
public class ParserTesting : MonoBehaviour
{
    ...
    public void RunFunctionHashMap()
    {
        string code = @"                

            function add_value_n(this, n)
                this.value = this.value + n
            end

            function add_value(this) 
                this.value = this.value + 1
                this.add_value_n(10)
            end    

            function new() 
                return { value=1, add_value=add_value, add_value_n=add_value_n}
            end
            return new
        ";
        ...
        CodeExecutor codeExecutor = new CodeExecutor(_ =>
        {
            ...
            CodeExecutor.CallFunction(handler, null, (node) =>
            {
                Table table = node as Table;
                CodeExecutor.CallFunction(table.Get(new String("add_value")) as FunctionHashMap, null, __ =>
                {    
                    Debug.Log($"Last result: {table}");
                });
            });
